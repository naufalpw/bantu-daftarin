...
@section('content')
    ...
    <nav class="bd-phone-only bd-phone-section-links" aria-label="Bagian pengajuan">
        <a href="#documents-title">Dokumen</a>
        <a href="#admin-next-action">Tindakan berikutnya</a>
        <a href="#history-title">Riwayat</a>
    </nav>
    <div id="admin-primary-operation" class="bd-phone-operation"></div>
    <div class="bd-admin-detail-grid mt-8">
        <div class="bd-admin-detail-content">
            ...
            <details open data-phone-disclosure class="bd-admin-surface bd-admin-detail-section--payment" aria-labelledby="payment-title">
            ...
            <details open data-phone-disclosure class="bd-admin-surface bd-admin-detail-section--data" aria-labelledby="data-title">
                <summary>Data pengajuan</summary>
            ...
            <section class="bd-admin-surface bd-admin-detail-section--documents" aria-labelledby="documents-title">
                ...
                <div class="bd-admin-document-list">
                    @foreach($application->requirements as $requirement)
                        ...
                        @php($documentTone = $activeDocument ? match($activeDocument->review_status->value) {'PENDING' => 'attention', 'REVISION_REQUIRED', 'REJECTED' => 'danger', 'LOCKED', 'ACCEPTED' => 'success', default => 'neutral'} : 'neutral')
                        <article class="bd-admin-document" data-document-tone="{{ $documentTone }}" @if($activeDocument) id="document-{{ $activeDocument->public_id }}" @endif>
                            <header><div><h3>{{ $requirement->name }}</h3><p>{{ $requirement->is_required ? 'Wajib' : 'Opsional' }} · {{ $requirement->statusLabel() }}</p></div><x-admin.status-badge :label="$activeDocument?->review_status->label() ?? 'Belum diunggah'" :tone="$documentTone" /></header>
                            @if($activeDocument)
                                ...
                                <div class="bd-admin-document__actions"><a href="{{ route('admin.documents.view', $activeDocument->public_id) }}" target="_blank" rel="noopener"><x-ui-icon class="bd-desktop-action-icon" name="open" :size="15" />Lihat dokumen</a><a href="{{ route('admin.documents.download', $activeDocument->public_id) }}"><x-ui-icon class="bd-desktop-action-icon" name="download" :size="15" />Unduh</a></div>
                                @if($application->status->value === 'UNDER_REVIEW' && $activeDocument->review_status->value === 'PENDING')
                                    @php($restoreReview = old('_ui_form') === 'document-'.$activeDocument->public_id)
                                    <details class="bd-admin-review-form"><summary>Catat keputusan</summary><form method="post" action="{{ route('admin.documents.review', $activeDocument->public_id) }}">@csrf<x-mobile-form-feedback :form-key="'document-'.$activeDocument->public_id" /><label for="action-{{ $activeDocument->id }}">Keputusan</label><select id="action-{{ $activeDocument->id }}" name="action"><option value="ACCEPT" @selected($restoreReview && old('action') === 'ACCEPT')>Terima dokumen</option><option value="REQUEST_REVISION" @selected($restoreReview && old('action') === 'REQUEST_REVISION')>Minta perbaikan</option><option value="REJECT" @selected($restoreReview && old('action') === 'REJECT')>Tolak dokumen</option></select><label for="reason-{{ $activeDocument->id }}">Alasan bila ditolak atau perlu diperbaiki</label><textarea id="reason-{{ $activeDocument->id }}" name="reason" rows="2" aria-invalid="{{ $restoreReview && $errors->any() ? 'true' : 'false' }}" @if($restoreReview && $errors->any()) aria-describedby="feedback-document-{{ $activeDocument->public_id }}" @endif>{{ $restoreReview ? old('reason') : '' }}</textarea><label for="instruction-{{ $activeDocument->id }}">Instruksi revisi</label><textarea id="instruction-{{ $activeDocument->id }}" name="instruction" rows="2" aria-invalid="{{ $restoreReview && $errors->any() ? 'true' : 'false' }}" @if($restoreReview && $errors->any()) aria-describedby="feedback-document-{{ $activeDocument->public_id }}" @endif>{{ $restoreReview ? old('instruction') : '' }}</textarea><button class="bd-admin-button bd-admin-button--primary" type="submit">Simpan keputusan</button></form></details>
                                @endif
            ...
            <div id="admin-review-operation" class="bd-phone-operation"></div>
            <section class="bd-admin-surface bd-admin-detail-section--result" aria-labelledby="result-title">
                ...
                @if($application->resultDocuments->isNotEmpty())<div class="bd-admin-document-list">@foreach($application->resultDocuments as $result)@php($resultTone = match($result->verification_status->value) {'VERIFIED' => 'success', 'REJECTED' => 'danger', default => 'attention'})<article class="bd-admin-document" data-document-tone="{{ $resultTone }}"><header><div><h3>{{ str_replace('_', ' ', strtolower($result->type->value)) }}</h3><p>Diunggah {{ $result->uploaded_at->translatedFormat('d M Y, H:i') }}</p></div><x-admin.status-badge :label="$result->verification_status->value === 'VERIFIED' ? 'Terverifikasi' : ($result->verification_status->value === 'REJECTED' ? 'Ditolak' : 'Menunggu verifikasi')" :tone="$resultTone" /></header><div class="bd-admin-document__actions"><a href="{{ route('admin.results.view', $result->public_id) }}" target="_blank" rel="noopener"><x-ui-icon class="bd-desktop-action-icon" name="open" :size="15" />Lihat hasil</a><a href="{{ route('admin.results.download', $result->public_id) }}"><x-ui-icon class="bd-desktop-action-icon" name="download" :size="15" />Unduh</a></div>@if($result->rejection_reason)<p class="bd-admin-inline-note bd-admin-inline-note--danger"><strong>Alasan:</strong> {{ $result->rejection_reason }}</p>@endif@if(in_array($result->verification_status->value, ['PENDING', 'REJECTED'], true) && $application->status->value === 'RESULT_REVIEW')<details class="bd-admin-review-form"><summary>Verifikasi hasil</summary><form method="post" action="{{ route('admin.results.verify', $result->public_id) }}">@csrf<label for="verified-{{ $result->id }}">Keputusan</label><select id="verified-{{ $result->id }}" name="verified"><option value="1">Verifikasi hasil</option><option value="0">Tolak hasil</option></select><label for="result-reason-{{ $result->id }}">Alasan bila ditolak</label><textarea id="result-reason-{{ $result->id }}" name="reason" rows="2"></textarea><button class="bd-admin-button bd-admin-button--primary" type="submit">Simpan verifikasi</button></form></details>@endif</article>@endforeach</div>@else<div class="bd-admin-empty-state bd-admin-empty-state--compact"><h3>Belum ada hasil</h3><p>Hasil akan dicatat di sini setelah diunggah melalui tahap proses yang valid.</p></div>@endif
            </section>

            <div id="admin-result-operation" class="bd-phone-operation"></div>
            <details open data-phone-disclosure class="bd-admin-surface bd-admin-detail-section--history" aria-labelledby="history-title"><summary>Riwayat pengajuan</summary><div class="bd-admin-surface__header"><div><h2 id="history-title">Riwayat pengajuan</h2><p>Riwayat status dan estimasi</p></div></div><ol class="bd-admin-timeline">@forelse($application->statusHistories->sortByDesc('created_at') as $history)<li><strong>{{ $history->to_status->label() }}</strong><span>{{ $history->created_at->translatedFormat('d M Y, H:i') }}</span>@if($history->reason)<p>{{ $history->reason }}</p>@endif</li>@empty<li><strong>Belum ada riwayat status</strong></li>@endforelse @foreach($application->estimateHistories->sortByDesc('created_at') as $estimate)<li><strong>Estimasi diperbarui</strong><span>{{ $estimate->created_at->translatedFormat('d M Y, H:i') }}</span><p>{{ $estimate->reason }}</p></li>@endforeach</ol></details>
        </div>

        <aside class="bd-admin-detail-aside">
            <section id="admin-next-action" tabindex="-1" data-phone-move-to="{{ $application->status->value === 'UNDER_REVIEW' ? '#admin-review-operation' : ($application->status->value === 'RESULT_REVIEW' ? '#admin-result-operation' : '#admin-primary-operation') }}" class="bd-admin-surface"><div class="bd-admin-surface__header"><div><h2>Tindakan berikutnya</h2><p>{{ $nextAction['description'] }}</p></div></div>
                <div class="bd-admin-action-stack">
                    ...
                    @elseif($application->status->value === 'COMPLETED')<form method="post" action="{{ route('admin.applications.archive', $application->public_id) }}" data-confirm="Arsipkan pengajuan ini?">@csrf<button class="bd-admin-button bd-admin-button--secondary bd-admin-button--wide">Arsipkan pengajuan</button></form>@else<p class="bd-admin-muted-copy">Tidak ada tindakan admin yang tersedia pada tahap ini.</p>@endif
                </div>
            </section>
