...
class AdminPhaseBTest extends TestCase
{
    ...
    public function test_dashboard_uses_exact_operational_states_and_unread_client_messages(): void
    {
        ...
        $this->actingAs($admin->user)
            ...
            ->assertSee('Ada pekerjaan yang perlu ditindaklanjuti')
            ->assertSee('Buka area dengan antrean untuk melanjutkan pekerjaan.')
            ->assertSee('data-admin-command-board', false)
            ->assertSee('data-admin-attention-count="2"', false)
            ->assertSee(route('admin.applications.index', ['filter' => 'review']), false)
            ->assertSee(route('admin.documents.index', ['filter' => 'revision']), false)
            ->assertSee(route('admin.applications.index', ['filter' => 'result']), false)
            ->assertSee(route('admin.support.index', ['filter' => 'unread']), false)
    ...
    public function test_dashboard_uses_honest_empty_operational_states(): void
    {
        ...
        $this->actingAs($admin->user)
            ...
            ->assertSee('Tidak ada antrean prioritas')
            ->assertSee('Semua area operasional sedang terkendali.')
            ->assertSee('data-admin-attention-count="0"', false)
            ->assertDontSee('Ada pekerjaan yang perlu ditindaklanjuti')
